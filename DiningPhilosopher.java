        ...
        public void run() {
            try{
                ...
                while(mealsRemaining > 0) {
                    //lets do one case if there is only one philosopher
                    if(numPhilosophers == 1 ){
                        chops[0].acquire();
                        mutexLog("---", "Philosopher " + id + "'s left chopstick IS available.");
                        mutexLog("---" ,"Philosopher " + id + "'s right chopstick IS available.");
